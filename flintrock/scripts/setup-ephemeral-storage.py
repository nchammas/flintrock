...
BlockDevice.__new__.__defaults__ = (None, 0, None)
...
def get_non_root_block_devices():
    ...
    """
    min_size_of_disk_in_bytes = 10000000000
    block_devices_raw = subprocess.check_output([
        'lsblk',
        '--ascii',
        '--paths',
        '-b',
        '--output', 'KNAME,SIZE,MOUNTPOINT',
    ...
    non_root_block_devices = [bd for bd in block_devices if bd.mount_point != '/' and int(bd.size) >= min_size_of_disk_in_bytes]
